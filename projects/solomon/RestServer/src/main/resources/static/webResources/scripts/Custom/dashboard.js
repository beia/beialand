document.addEventListener('DOMContentLoaded',function(){
  ...
  document.getElementById("logOutBtn").addEventListener('click', function(){
    window.location.replace("/solomon/logout");
  },false);
  ...
  document.getElementById("historyBtn").addEventListener('click', function(){
    window.location.href = "/solomon/history";
  },false);

  document.getElementById("contactBtn").addEventListener('click', function(){
    window.location.href = "/solomon/contact";
  },false);
  ...
  document.getElementById("search-input").addEventListener('input', function(){
    ...
    for(elem in json){
      if(json[elem].title.toLowerCase().includes(inValue.toLowerCase())){
        drawElementInList(elem)
      }
    }
  })
...
function nsModalSave(){
  ...
  requestData = {};
  if(edit == "true"){
    requestData["requestUrl"] = "/solomon/retailer/updateCampaign";
    requestData["id"] = document.getElementById("itemID").value;
  } else {
    requestData["requestUrl"] = "/solomon/retailer/addCampaign";
  }
  ...
  requestData["startDate"] = $("#rangestart").calendar('get date').toISOString().split("T")[0]+"T00:00:00";
  requestData["endDate"] = $("#rangeend").calendar('get date').toISOString().split("T")[0]+"T23:59:59";
  ...
  request.done(function(response, textStatus, jqXHR){
      ...
      json[response["id"]] = {};
      json[response["id"]]["campaignID"] = response["id"];
      json[response["id"]]["title"] = response["title"];
      json[response["id"]]["category"] = response["category"];
      json[response["id"]]["description"] = response["description"];
      json[response["id"]]["startDate"] = response["startDate"].substring(0,10);
      json[response["id"]]["endDate"] = response["endDate"].substring(0,10);
      json[response["id"]]["image"] = "data:*/*;base64," +response["image"];

      drawElementInList(response["id"]);
      drawRightSide(response["id"]);
...
function rmvModalYes(){
  ...
  request.done(function(response, textStatus, jqXHR){
    ...
    document.getElementById("rightCategory").innerHTML = "";
...
function drawList(){
  ...
  var request = $.ajax({
    type: "get",
    url: "/solomon/retailer/campaigns",
  ...
  request.done(function(response, textStatus, jqXHR){
    for(var i=0; i<response.length; i++) {
      json[response[i]["id"]] = {};
      json[response[i]["id"]]["campaignID"] = response[i]["id"];
      json[response[i]["id"]]["title"] = response[i]["title"];
      json[response[i]["id"]]["category"] = response[i]["category"];
      json[response[i]["id"]]["description"] = response[i]["description"];
      json[response[i]["id"]]["startDate"] = response[i]["startDate"].substring(0,10);
      json[response[i]["id"]]["endDate"] = response[i]["endDate"].substring(0,10);
      json[response[i]["id"]]["image"] = "data:*/*;base64," +response[i]["image"];

      drawElementInList(response[i]["id"]);
    }
  });
}
...
function drawElementInList(campaignID){
  ...
  table.innerHTML = "<tr> \
                          <td class='collapsing'> \
                            <img src= '" + json[campaignID]["image"] + "' class='image thumb-im'> \
                          </td> \
                          <td><strong>" + json[campaignID]["title"] + "</strong><br>" + json[campaignID]["startDate"] + " <-> " + json[campaignID]["endDate"] + "</td> \
                        </tr>"
