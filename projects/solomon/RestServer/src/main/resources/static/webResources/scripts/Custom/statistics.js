...
function getStatistics(){
	...
	var request = $.ajax({
		type: "get",
		url: "/solomon/retailer/getViewStats",
    ...
    request.done(function(response, textStatus, jqXHR){
      ...
      if(response.length > 0){
      	...
      	responseData["campaignViews"] = [];

      	for(var i=0; i<response.length; i++){
      		var view = {};
      		view["idCampaign"] = response[i].campaign["id"];
			view["idUser"] = response[i].user["id"];
			view["gender"] = response[i].user["gender"];
			view["age"] = response[i].user["age"];
			view["viewDate"] = response[i].date;

			responseData["campaignViews"].push(view);
		}
        document.getElementById("viewStatisticsTable").hidden = false;
        getUsersStatistics(new Date(json[document.getElementById("itemID").value]["startDate"]), new Date(),responseData);
        getViewsByDay(new Date(json[document.getElementById("itemID").value]["startDate"]), new Date(),responseData);
      } else {
        document.getElementById("viewStatisticsTable").hidden = true;
      }
    });
