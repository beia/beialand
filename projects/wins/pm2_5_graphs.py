...
PM2_5_time.insert(0,'24-10-18')

unic_days = []
unic_days.append(PM2_5_time[0][0:8])
...
for d in unic_days:
    start = 0
    stop = 0
    for i in range(len(PM2_5_time)):
        if PM2_5_time[i][0:8] == d:
            start = i
        elif PM2_5_time[i][0:8] == unic_days[unic_days.index(d)+1]:
            stop = i
    ...
    for i in range(start + 1, stop + 1):
        # print(PM10[i])
        x_data.append(i)
        y_data.append(PM2_5_data[i])
        x_data_days_hours.append(PM2_5_time[i] + " " + PM2_5_time[i])
    ...
    plt.ylim(top=200)
    plt.xlabel('data')
    plt.ylabel('grad de poluare PM2_5')
