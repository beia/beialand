...
namespace Safecare.BeiaDeviceDriver_Fan
{
    ...
    internal class InputPoller
    {
        ...
        public InputPoller(IEventManager eventManager, BeiaDeviceDriver_FanConnectionManager connectionManager,
                ...
                try
                {
                    if (_messageHandler.Initialized && _lastUpdateTime < _messageHandler.Data.Time)
                    {
                        ...
                        _eventManager.NewEvent(Constants.DriverId.ToString(), EventId.OutputChanged,
                            new System.Collections.Generic.Dictionary<string, string> { { "Fan", msg } });
